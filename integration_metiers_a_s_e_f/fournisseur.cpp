...
void fournisseur::rechercherFournisseurParId()
{
    ...
    if (query.next()) {
        ...
        page->findChild<QDateEdit*>("deDatePartenariat_9")->setDate(QDate::fromString(query.value("DATE_DE_PARTENERIAT").toString(), "yyyy-MM-dd"));
...
void fournisseur::rafraichirListeFournisseurs()
{
    ...
    table->clearContents();           // deletes all items
    table->setRowCount(0);            // removes all rows
    table->setColumnCount(9);        // ID + 7 fields + Score
    ...
    while (query.next()) {
        ...
        // Create NEW items every time → no dangling pointers
        ...
        // Make all columns read-only except score if you want
