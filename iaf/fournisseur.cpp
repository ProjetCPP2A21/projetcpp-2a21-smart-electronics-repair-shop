...
{
    auto idEdit = page->findChild<QLineEdit*>("leld_9");
    if (!idEdit) return -1;
...
{
    auto nomEdit = page->findChild<QLineEdit*>("leNomEntreprise_9");
    auto nomFEdit = page->findChild<QLineEdit*>("leContactNom_9");
    auto prenomFEdit = page->findChild<QLineEdit*>("leContactPrenom_9");
    auto telEdit = page->findChild<QLineEdit*>("leTelephone_9");
    auto emailEdit = page->findChild<QLineEdit*>("leEmail_9");
    auto typeEdit = page->findChild<QLineEdit*>("leTypePieces_9");
    auto adrEdit = page->findChild<QTextEdit*>("teAdresse_9");
    auto dateEdit = page->findChild<QDateEdit*>("deDatePartenariat_9");
...
// RECHERCHER PAR ID
void fournisseur::rechercherFournisseurParId(QWidget *page)
{
    ...
    QSqlQuery query;
    query.prepare(
        "SELECT ID_FOURNISSEUR, NOM_ENT, NOM_F, PRENOM_F, TEL, EMAIL, "
        "TYPES_DES_PIECES, ADRESSE, DATE_DE_PARTENERIAT "
    ...
    if (!query.exec()) {
        QMessageBox::critical(page, "Erreur",
                              "Recherche impossible : " + query.lastError().text());
        return;
    }
    ...
    if (query.next()) {
        ...
        // Remplir champs
        page->findChild<QLineEdit*>("leNomEntreprise_9")->setText(query.value("NOM_ENT").toString());
        page->findChild<QLineEdit*>("leContactNom_9")->setText(query.value("NOM_F").toString());
        page->findChild<QLineEdit*>("leContactPrenom_9")->setText(query.value("PRENOM_F").toString());
        page->findChild<QLineEdit*>("leTelephone_9")->setText(query.value("TEL").toString());
        page->findChild<QLineEdit*>("leEmail_9")->setText(query.value("EMAIL").toString());
        page->findChild<QLineEdit*>("leTypePieces_9")->setText(query.value("TYPES_DES_PIECES").toString());
        page->findChild<QTextEdit*>("teAdresse_9")->setPlainText(query.value("ADRESSE").toString());
        page->findChild<QDateEdit*>("deDatePartenariat_9")->setDate(
            QDate::fromString(query.value("DATE_DE_PARTENERIAT").toString(), "yyyy-MM-dd")
            );

        // ==== HIGHLIGHT ROW IN TABLE ====
        QTableView *tv = page->findChild<QTableView*>("tabaff_6");
        if (tv && tv->model()) {

            int rows = tv->model()->rowCount();

            for (int r = 0; r < rows; r++) {
                QModelIndex idIndex = tv->model()->index(r, 0); // 0 = colonne ID_FOURNISSEUR
                if (idIndex.data().toInt() == id) {
                    tv->selectRow(r);
                    tv->scrollTo(idIndex);
                    break;
                }
...
void fournisseur::modifierFournisseur()
{
    ...
    QString nom = page->findChild<QLineEdit*>("leNomEntreprise_9")->text().trimmed();
    QString nomF = page->findChild<QLineEdit*>("leContactNom_9")->text().trimmed();
    QString prenomF = page->findChild<QLineEdit*>("leContactPrenom_9")->text().trimmed();
    QString telephone = page->findChild<QLineEdit*>("leTelephone_9")->text().trimmed();
    QString email = page->findChild<QLineEdit*>("leEmail_9")->text().trimmed();
    QString typePieces = page->findChild<QLineEdit*>("leTypePieces_9")->text().trimmed();
    QString adresse = page->findChild<QTextEdit*>("teAdresse_9")->toPlainText().trimmed();
    QString date = page->findChild<QDateEdit*>("deDatePartenariat_9")->date().toString("yyyy-MM-dd");
    ...
    page->findChild<QLineEdit*>("leContactNom_9")->setText(currentNomF);
    page->findChild<QLineEdit*>("leContactPrenom_9")->setText(currentPrenomF);
...
void fournisseur::rafraichirListeFournisseurs()
{
    ...
    // CRITICAL FIX: Clear everything properly
    table->clearContents();           // efface tous
    table->setRowCount(0);            // efface tous les lignes
    table->setColumnCount(9);        // ID + 7 ligne + Score
    ...
    QSqlQuery query("SELECT ID_FOURNISSEUR, NOM_ENT, NOM_F, PRENOM_F, TEL, EMAIL, "
                    "TYPES_DES_PIECES, DATE_DE_PARTENERIAT FROM FOURNISSEUR ORDER BY NOM_ENT");

    int row = 0;
    while (query.next()) {
        table->insertRow(row);

        // creations des new cellules
        ...
        table->setItem(row, 7, new QTableWidgetItem(query.value("DATE_DE_PARTENERIAT").toString().left(10)));

        // Score colonnes
        int id = query.value("ID_FOURNISSEUR").toInt();
        ScoreFournisseur sc = calculerScoreFournisseur(id);
        QTableWidgetItem* scoreItem = new QTableWidgetItem(QString::number(sc.scoreTotal));
        scoreItem->setTextAlignment(Qt::AlignCenter);
        scoreItem->setFlags(scoreItem->flags() & ~Qt::ItemIsEditable);
        table->setItem(row, 8, scoreItem);
...
void fournisseur::viderChamps()
{
    ...
    auto nom = page->findChild<QLineEdit*>("leNomEntreprise_9"); if (nom) nom->clear();
    auto cnom = page->findChild<QLineEdit*>("leContactNom_9"); if (cnom) cnom->clear();
    auto cprenom = page->findChild<QLineEdit*>("leContactPrenom_9"); if (cprenom) cprenom->clear();
    auto tel = page->findChild<QLineEdit*>("leTelephone_9"); if (tel) tel->clear();
    auto mail = page->findChild<QLineEdit*>("leEmail_9"); if (mail) mail->clear();
    auto type = page->findChild<QLineEdit*>("leTypePieces_9"); if (type) type->clear();
    auto adr = page->findChild<QTextEdit*>("teAdresse_9"); if (adr) adr->clear();
    auto date = page->findChild<QDateEdit*>("deDatePartenariat_9"); if (date) date->setDate(QDate::currentDate());
}
...
void fournisseur::afficherFournisseursPrioritairesMA2()
{
    ...
    for (int r = 0; r < table->rowCount(); ++r) {
        for (int c = 0; c < table->columnCount(); ++c) {
            if (table->item(r, c))
                table->item(r, c)->setBackground(Qt::transparent);
        }
    }

    QList<FournisseurCritique> list = identifierFournisseursPrioritaires();
    if (list.isEmpty()) {
        QMessageBox::information(page, "MA2", "Aucun fournisseur critique détecté.");
        return;
    }
    ...
    for (int row = 0; row < table->rowCount(); ++row) {
        ...
        auto it = std::find_if(list.begin(), list.end(),
                               [id](const FournisseurCritique& fc) { return fc.id == id; });
        if (it == list.end()) continue;

        QColor color;
        double score = it->scoreCriticite;
        if (score >= 30)       color = QColor("#ff4444");      // red
        else if (score >= 15)  color = QColor("#44ff44");      // green
        else                   color = QColor("#4444ff");      // blue
    ...
    QMessageBox::information(page, "MA2 – Priorité",
                             "Couleurs appliquées :\n"
                             "• Rouge  → Très critique\n"
                             "• Vert   → Prioritaire\n"
                             "• Bleu   → À surveiller");
